...
Moreover, if we model $f(X)$ accurately and draw new samples from this model, our original data should be protected sufficiently well, because many real-world processes are far from deterministic. 
These random fluctuations typically provide sufficient uncertainty to prevent both identity and attribute disclosure.
<!-- But will we also *check* the level of disclosure, or simply trust that it's "typically sufficient"? -->
...
<!-- Question: In the example, the Fully conditional specification assumes mvnorm - which has been criticised in the section above. Does synthpop allow non-normal distirbutions? -->
...
<!-- Here it would be nice if the results can be printed in a collapsed box (for those who do not run the code themselves) -->
...
<!-- This renders to "As stated in Section 2,", but we have no section numbers, so this is not meaningful -->
...
<!-- Do we get advice on how to find this trade-off? And how to judge whether we found it? Any advice in the tutorial, or pointing to external ressources? -->
...
<!-- But how can I write up such a section as the data provider? How do I know "what your synthetic data set can and cannot be used for"? -->
