...
In 2006, researchers from the University of Texas were able to uncover the identity of Netflix users by linking Netflix movie reviews to the Internet Movie Database (IMDb).
This might not be a problem for most users, but turned out to be problematic for some.
For example, the reviewed movies were quite explicit about some users' sexual orientation, and revealing this information could put some in an awkward position.
...
For example, one might threshold variables such that extreme values are not released, which typically creates bias in the distribution of the released values.
