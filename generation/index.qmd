...
Additionally, we will use the additional package `densityratio` [@densityratio] to evaluate the utility of synthetic data.
...
Either a single character string with a synthesis model that is applied to all variables, or a character vector with a synthesis model per variable. The default method is use classification and regression trees [for an introduction, see, for instance, Section 8.1 in @islr] for all variables, which is a flexible, non-parametric technique for modelling synthetic data. Other synthesis models that are available are listed below (for a more thorough explanation, see `?synthpop::syn.METHOD`, with `METHOD` replaced by the actual method as specified below).
...
To this end, we need to decide on a parametric model to synthesize each variable, which can be done my specifying `method = "parametric"` in the call to `syn()`.
...
# Final thoughts on specifying conditional synthesis models {#sec-synth-model-guidelines}

In the subsequent sections, we do not generate synthetic data by writing all code from scratch, but use readily available functions that simplify the workflow. Note, however, that you can always fall back to these manual procedures if the available software does not allow you implement the synthesis model that you wish._
